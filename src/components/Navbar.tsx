...
export default function Navbar() {
  ...
  const Logo = () => (
    <a
      href="/"
      onClick={(e) => { e.preventDefault(); setMobileOpen(false); router.push("/"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
    ...
    <>
      ...
      >
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
              ...
              return (
                <a
                  ...
                  className={`relative text-sm transition-colors ${
                    mounted && isActive ? "text-accent" : "text-muted hover:text-foreground"
                  }`}
                >
                  {t(labelKey)}
                  {mounted && isActive && (
          ...
          >
            <motion.span
              animate={{ opacity: mobileOpen ? 0 : 1, width: mobileOpen ? 0 : "auto" }}
            ...
            <motion.span
              animate={{ rotate: mobileOpen ? 45 : 0, opacity: mobileOpen ? 1 : 0, width: mobileOpen ? "auto" : 0 }}
      ...
      {mounted && createPortal(
        ...
        {mobileOpen && <motion.div
          ...
          >
            ...
            <div aria-hidden="true" className="absolute top-20 left-5 w-6 h-6 border-t border-l border-accent/40" />
            <div aria-hidden="true" className="absolute top-20 right-5 w-6 h-6 border-t border-r border-accent/40" />
            <div aria-hidden="true" className="absolute bottom-10 left-5 w-6 h-6 border-b border-l border-accent/40" />
            <div aria-hidden="true" className="absolute bottom-10 right-5 w-6 h-6 border-b border-r border-accent/40" />

            {/* Nav items */}
            <div className="flex-1 flex flex-col justify-center px-10 gap-2 mt-16">
              <p className="font-mono text-xs text-accent/50 mb-6 tracking-widest">// NAVIGATION</p>
              {NAV_SECTIONS.map(({ id, labelKey }, i) => {
                ...
                return (
                  <motion.a
                    ...
                    style={isActive ? { textShadow: "0 0 18px rgba(0,229,255,0.7)" } : undefined}
                  >
